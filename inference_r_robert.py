        ...
        with torch.no_grad():
            if is_roberta:
                outputs = model(
                    ...
                    e1_mask=data['e1_mask'].to(device),  ## r_robert
                    e2_mask=data['e2_mask'].to(device)  ## r_robert
                )
            else:
                outputs = model(
                    input_ids=data['input_ids'].to(device),
...
def main(args):
    ...
    if args.k_fold:
        ...
        model = MyModel(args.PLM, config=model_config, dropout_rate=0.1)
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'pytorch_model.pt')))  ## r_robert
        model.parameters
        model.to(device)
