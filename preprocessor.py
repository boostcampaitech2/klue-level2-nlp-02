...
def sentence_processing(data):
    ...
    for row in data.values:
        ...
        if sub_start_idx < ob_start_idx:
            sentence = sentence[:sub_start_idx] + '@ * ' + TAG_DICT[sub_type] + ' * ' + sentence[sub_start_idx:sub_end_idx+1] + ' @ ' + \
                sentence[sub_end_idx+1:ob_start_idx] + '# ~ ' + TAG_DICT[ob_type] + ' ~ ' + \
                sentence[ob_start_idx:ob_end_idx+1] + \
                ' # ' + sentence[ob_end_idx+1:]
        else:
            sentence = sentence[:ob_start_idx] + '# ~ ' + TAG_DICT[ob_type] + ' ~ ' + sentence[ob_start_idx:ob_end_idx+1] + ' # ' + sentence[ob_end_idx +
                                                                                                                                    1:sub_start_idx] + '@ * ' + TAG_DICT[sub_type] + ' * ' + sentence[sub_start_idx:sub_end_idx+1] + ' @ ' + sentence[sub_end_idx+1:]
